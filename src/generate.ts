...
export async function generate(options: GenerateOptions) {
  ...
  const rawValidatorDefinitionsOutput = validators.validatorNames.map(
    (name) => `export function ${name}(json: unknown): boolean;`
  ).join('\n');

  const validatorDefinitionsOutput = format(rawValidatorDefinitionsOutput, prettierOptions);
...
function generateValidators(schema: ParsedSchema): ParsedValidators {
  const ajv = new Ajv({ code: { source: true }, strict: false });
  ajv.compile(JSON.parse(schema.json));

  const validatorConfig: Record<string, string> = {};
  ...
  Object.entries(schema.definitions)
    ...
    .map(([definitionName]) => {
      ...
      validatorConfig[validatorName] = `#/definitions/${definitionName}`;
    });
  ...
  // .replace(/(\w+)(\.errors)/gm, "($1 as any)$2")
  // .replace(/"use strict"/g, "")
  // .replace(/parentData,/gm, "parentData = undefined,")
  // .replace(/parentDataProperty,/gm, "parentDataProperty = undefined,")
